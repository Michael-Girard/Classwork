...
public class Chapter40Assignment extends Application{
   private static Tree24 myTree = new Tree24();
   ...
   public void start(Stage primaryStage){
      ...
      treePane.setPrefWidth(1300);
      
      //controlPane styling and content insertion
      controlPane.setHgap(5);
      controlPane.setAlignment(Pos.CENTER);
      controlPane.getChildren().addAll(buttonDelete, tfNumber, buttonInsert);
      
      mainPane.getChildren().addAll(treePane, controlPane);
      
      // Display
      Scene scene = new Scene(mainPane, 1300, 400);
      primaryStage.setTitle("2-4 Tree");
      primaryStage.setScene(scene);
      primaryStage.show();
      
      buttonDelete.setOnAction( e-> {
         treePane.getChildren().clear();
         myTree.delete(Integer.valueOf(tfNumber.getText()));
         tfNumber.setText("");
         if (myTree.getRoot() != null){
            displayTree(myTree.getRoot(), 1, treePane.getWidth() / 2 - 50);
         }
      });
      
      buttonInsert.setOnAction( e-> {
         treePane.getChildren().clear();
         myTree.insert(Integer.valueOf(tfNumber.getText()));
         tfNumber.setText("");
         if (myTree.getRoot() != null){
            displayTree(myTree.getRoot(), 1, treePane.getWidth() / 2 - 50);
         }
   ...
   private static void displayTree(Tree24.Tree24Node node, int layer, double circleX){
      StringBuilder builder = new StringBuilder();
      ArrayList elementList = node.getElements();
      for (int index = 0; index < elementList.size(); index++){
         builder.append(elementList.get(index).toString());
      ...
      Circle circle = new Circle(circleX, layer * 90, 30);
      circle.setFill(Color.WHITE);
      circle.setStroke(Color.BLACK);
      Label elements = new Label(builder.toString());
      treePane.getChildren().addAll(circle, elements);
      elements.setTranslateX(circleX - 7 * elementList.size());
      elements.setTranslateY(layer * 90 - 10);
      if (layer != 1){
         
      }
      ArrayList<Tree24.Tree24Node> childList = node.getChildren();
      for (int index = 0; index < childList.size(); index++){
         if (index < childList.size() / 2){
            Line connector = new Line(circleX - 20, layer * 90 + 24, 
               ((circleX - (300 - layer * 80)) + (index * (100 - 20 * layer))) + 15, 
               ((layer + 1) * 90 - 10));
            treePane.getChildren().add(connector);
            displayTree(childList.get(index), layer + 1, (circleX - (300 - layer * 80)) + (index * (100 - 20 * layer)));
         }
         else{
            Line connector = new Line(circleX + 20, layer * 90 + 24, 
               ((circleX + (300 - layer * 80)) + (index * (100 - 20 * layer))) - 15, 
...
class Tree24<E extends Comparable<E>> implements Tree<E> {
   ...
   */
   ...
   protected static class Tree24Node<E extends Comparable<E>> {
      ...
      public ArrayList<Tree24Node<E>> getChildren(){
         return child;
      }
   }
}